...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (unref(statistics)) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "page relative h-full w-full transition-width flex flex-col overflow-hidden items-stretch flex-1" }, _attrs))}><div class="pt-10 w-full mx-auto max-w-5xl"><h3 class="text-2xl font-semibold mb-6">Profile Statistics</h3><div class="flex flex-col gap-6 p-6 bg-[#444653] rounded-3xl overflow-y-auto mb-4"><div class="grid grid-cols-2 gap-6"><div class="flex flex-col text-center"><b class="text-xl">${ssrInterpolate(unref(statistics).training.total)}</b><small>Articles added</small></div><div class="flex flex-col text-center"><b class="text-xl">${ssrInterpolate(`${unref(statistics).validation.validated.total} / ${unref(statistics).validation.articlesCount}`)}</b><small>Validated articles</small></div></div><div class="grid grid-cols-3 gap-6"><div class="flex flex-col text-center p-4 bg-green-600 rounded-xl font-semibold"><b class="text-xl">${ssrInterpolate(unref(statistics).validation.validated.accepted)}</b><small>Validation accepted</small></div><div class="flex flex-col text-center p-4 bg-red-600 rounded-xl font-semibold"><b class="text-xl">${ssrInterpolate(unref(statistics).validation.validated.rejected)}</b><small>Validation rejected</small></div><div class="flex flex-col text-center p-4 bg-sky-600 rounded-xl font-semibold"><b class="text-xl">${ssrInterpolate(unref(statistics).validation.validated.skiped)}</b><small>Validation skiped</small></div></div></div></div><div class="pt-10 w-full mx-auto max-w-5xl"><h3 class="text-2xl font-semibold mb-6">Parsers Statistics</h3><div class="flex flex-col gap-6 p-6 bg-[#444653] rounded-3xl overflow-y-auto mb-4"><div class="grid grid-cols-3 gap-6"><div class="flex flex-col text-center"><b class="text-xl">${ssrInterpolate(unref(statistics).parsers.parsed_24h)}</b><small>Parsed 24h</small></div><div class="flex flex-col text-center"><b class="text-xl">${ssrInterpolate(unref(statistics).parsers.parsed_7d)}</b><small>Parsed 7d</small></div><div class="flex flex-col text-center"><b class="text-xl">${ssrInterpolate(unref(statistics).parsers.parsed)}</b><small>Parsed Total</small></div></div></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
    };
...
//# sourceMappingURL=statistics-3cde275b.mjs.map
